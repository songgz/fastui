...
module Fastui
  class MGlossariesController < ApplicationController
    ...
    def index
      ...
      respond_with(@m_glossaries.to_json(:include => [:m_glossary_items,:actived, :org, :createdby, :updatedby]))
    end
    ...
    def create
      @m_glossary = MGlossary.new(params[:m_glossary])
    ...
    def update
      ...
      respond_with(@m_glossary) do |format|
        if @m_glossary.update_attributes(params[:m_glossary])
          format.json { render :json => {:success => true, :msg => 'ok'} }
        else
