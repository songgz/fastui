...
module Fastui
  class MMenuItemsController < ApplicationController
    ...
    def index
         ...
         respond_with(@m_menu_items.to_json(:include => [:m_window,:m_menu,:actived,:org,:createdby,:updatedby], :methods => [:leaf?,:child?]))
    end
    def show
    ...
    def edit
      @m_menu_item = MMenuItem.find(params[:id])
      respond_with(@m_menu_item.to_json(:include => [:m_window,:m_menu,:actived,:org,:createdby,:updatedby]))
    end
